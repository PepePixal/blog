<!-- obtener el titulo de la ruta y asignarlo a la variable $title -->
...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <!-- Desktop User Menu -->
            {{-- mostrar este dropdown solo cuando el usuario esté autenticado --}}
            @auth
                <flux:dropdown position="top" align="end">
                    {{-- botón con iniciales del perfil de usuario --}}
                    <flux:profile
            ...
            @else
                <flux:dropdown position="top" align="end">
                    {{-- botón con icon, de la libreria heroicons.com --}}
                    <flux:button
                        class="cursor-pointer"
                        icon:trailing="user"
                    />
                    ...
                    <flux:menu>
                        <flux:menu.radio.group>
                            <flux:menu.item :href="route('login')" icon="arrow-left-end-on-rectangle" wire:navigate>
                                {{-- sintaxis para traducciones --}}
                                {{ __('Log In') }}
                            </flux:menu.item>

                            <flux:menu.separator />
                            
                            <flux:menu.item :href="route('register')" icon="inbox-stack" wire:navigate>
                                {{-- sintaxis para traducciones --}}
                                {{ __('Register') }}
                            </flux:menu.item>
                        </flux:menu.radio.group>
