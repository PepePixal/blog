<x-layouts.app>
    ...
    <div class="px-4 mx-auto max-w-screen-xl">
        ...
        <div class="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
            {{-- Recorrer los posts relacionados y por cada post mostrar su imagen, titulo y descripcion --}}
            @foreach ($relatedPosts as $post)
                <article class="max-w-xs">
                    <a href="{{ route('posts.show', $post) }}">
                        <img src="{{ $post->image }}" alt="no-image" class="mb-5 rounded-lg" alt="Image 1">
                    </a>
                    <h2 class="mb-2 text-xl font-bold leading-tight text-gray-900 dark:text-white">
                        <a href="{{ route('posts.show', $post) }}">{{ $post->title }}</a>
                    </h2>
                    <p class="mb-4 text-gray-500 dark:text-gray-400">{{ Str::limit($post->excerpt, 100) }}</p>
                    <a href="{{ route('posts.show', $post) }}" class="inline-flex items-center font-medium underline underline-offset-4 text-primary-600 dark:text-primary-500 hover:no-underline">
                        Leer más
                    </a>
                </article>
            @endforeach
